...
M-LINK data packets are always 8 bytes in size, and consist of a packet identifier byte (PID), 6 bytes payload (P0..P5), and a checksum byte (CC):

  0 |  1 |  2 |  3 |  4 |  5 |  6 | 7
----|----|----|----|----|----|----|---
PID | P0 | P1 | P2 | P3 | P4 | P5 | CC
...
In every channel data packet, three 16-bit channel values are transmitted. M-LINK can transmit up to sixteen channel values, for which six different PIDs are reserved. The channel values are encoded as follows:
  ...
  0 |  1  |  2  |  3  |  4  |  5  |  6  |  7
----|-----|-----|-----|-----|-----|-----|----
PID | D1H | D1L | D2H | D2L | D3H | D3L | CC
...
```text
Dx_value = (pulse_width - 800us) * 2.7648
```

The constant 2.7648 ticks/us is derived from the crystal frequency divided by 4, i.e. `11.0592MHz / 4`
...
- Normal mode: 21ms cycle time, 42ms transmit time, 16 channels
- Fast-response mode: 14ms cycle time, 28ms transmit time, 12 channels
...
During two normal cycles (without telemetry), the following channel data packets (PIDs) are transmitted:
...
Packets 0x09/0x01, 0x0A/0x02, and 0x88/0x80 alternate after each cycle. Bit 7 of the PID indicates a radio frequency change, which occurs at the end of every 21ms cycle.
...
the radio is switched to RX mode giving the receiver a window to transmit data. In a telemetry cycle, only the following packets (PID) are transmitted:
...
In fast-response mode, the 0x0A/0x02 packets are not included, i.e. the following packets (PID) are transmitted:
...
To transmit all 12 channels, 2 cycles (28ms) are needed.
...
A telemetry data frame sent by the receiver uses the PID 0x13 and contains two sensor values with three bytes each, depicted as S1x and S2x. The high-nibble of the first byte SxA contains the 4-bit sensor address, the low-nibble of the first byte SxA contains a 4-bit unit type. The second and and third byte contains the 16-bit sensor data word, depicted as SxL and SxH. The LSB of the sensor data word is an alarm bit, the remaining 15 bits contain the signed sensor value.
  ...
  0 |  1  |  2  |  3  |  4  |  5  |  6  |  7
----|-----|-----|-----|-----|-----|-----|----
PID | S1A | S1L | S1H | S2A | S2L | S2H | CC
